...
public class InventoryUtils {
    ...
    public static int search(Function<ItemStack, Boolean> valid, Inventory inv, int start, int end, List<Integer> avoid) {
        ...
        while (i < size && (avoid.contains(i) || !valid.apply(inv.getStack(i))))
            i++;
        return i == size ? -1 : i;
    }
    ...
    public static List<Integer> searchAll(Function<ItemStack, Boolean> valid, Inventory inv, int start, int end, int max, List<Integer> avoid) {
        ...
        for (int i = start; i < end && slots.size() <= max; i++) {
            if (!avoid.contains(i) && valid.apply(inv.getStack(i))) {
                slots.add(i);
            }
        }
        return slots;
    ...
    public static List<Integer> searchAll(Function<ItemStack, Boolean> valid, ScreenHandler screenHandler, int start, int end, int max, List<Integer> avoid) {
        ...
        List<Integer> slots = new ArrayList<>();
        for (int i = start; i < end && slots.size() <= max; i++) {
            if (!avoid.contains(i) && valid.apply(screenHandler.getSlot(i).getStack())) {
                slots.add(i);
            }
        }
        return slots;
    ...
    public static void selectSlot(int slot, boolean setLastSlot) {
        if (setLastSlot) lastSlot = mc.player.getInventory().getSelectedSlot();
    ...
    public static Pair<Integer, Double> getBestTool(BlockState state) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            float j = canHarvest ? 30f : 100f;
            double score = WorldUtils.getToolSpeed(state, stack) / j;
            if (score > bestScore) {
                bestScore = score;
                best = i;
            }
