...
/**
 * Fixed checkbox with proper hover detection
...
public class UICheckBox extends UIElement {
    ...
    public UICheckBox() {
        ...
        this.addMouseEvent((event) -> {
            // Only toggle if we're actually hovered
            // Coordinates are already scaled, and onClick already checks hover
        ...
        this.onHoverEnter(() -> {
            animate("hoverProgress", hoverProgress, 1.0f, 200, AnimationEasing.EASE_OUT_QUAD);
        });

        this.onHoverExit(() -> {
            animate("hoverProgress", hoverProgress, 0.0f, 200, AnimationEasing.EASE_IN_QUAD);
        });
    }
    ...
    public UICheckBox setActive(boolean state) {
        ...
        animate("checkProgress", checkProgress, targetProgress, 250, AnimationEasing.EASE_OUT_QUAD);
        return this;
    }
    ...
    protected void render(DrawContext ctx) {
        ...
        if (checkProgress > 0.01f) {
            float scale = 0.2f + (checkProgress * 0.8f);
            ...
            drawLine(ctx, x1, y1, x2, y2, checkColorWithAlpha);
            drawLine(ctx, x2, y2, x3, y3, checkColorWithAlpha);

            int glowColor = (checkColor & 0x00FFFFFF) | ((int)(checkProgress * 60) << 24);
            drawLine(ctx, x1-1, y1, x2-1, y2, glowColor);
            drawLine(ctx, x2-1, y2, x3-1, y3, glowColor);
            drawLine(ctx, x1+1, y1, x2+1, y2, glowColor);
            drawLine(ctx, x2+1, y2, x3+1, y3, glowColor);
