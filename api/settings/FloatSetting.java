...
public class FloatSetting extends AbstractSetting<Float> {
    ...
    public void load(JSONObject savedSettings) {
        if (savedSettings.has(getName())) {
            float loadedValue = savedSettings.getFloat(getName());
    ...
    public void draw(DrawContext ctx, UIOption uiOption) {
        ...
        if (!sliderOpt.isPresent() || !(sliderOpt.get() instanceof UISlider)) {
            ...
            UISlider slider = new UISlider(getName(), min, max, getValue());
            ...
            float settingValue = getValue();
            
            // Only update if changed
            if (slider.isDragging()) {
                // Slider is being dragged - update setting
                if (Math.abs(sliderValue - lastKnownValue) > 0.001f) {
                    setValue(sliderValue);
                    lastKnownValue = sliderValue;
                    
                    if (onModified != null) {
                        onModified.run();
                    }
                }
            } else if (Math.abs(settingValue - sliderValue) > 0.001f) {
